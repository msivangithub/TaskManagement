...
import com.office.taskmanager.LoginSetup.LoginActivity;
import com.office.taskmanager.Pojo.Task;
import com.office.taskmanager.Pojo.TaskUser;
import com.office.taskmanager.R;
import com.office.taskmanager.fragment.AddUserFragment;
import com.office.taskmanager.fragment.AllUsersFragement;
import com.office.taskmanager.fragment.DailyReportFragment;
import com.office.taskmanager.fragment.ProformaFragment;
import com.office.taskmanager.fragment.ReportsFragment;
import com.office.taskmanager.fragment.SectionInchargesFragment;
import com.office.taskmanager.fragment.TaskcreationFragment;
import com.office.taskmanager.fragment.feedingPaperFragment;
import com.office.taskmanager.fragmentssss.TaskDetails;
import com.office.taskmanager.fragmentssss.UserTaskDetails;
...
import com.office.taskmanager.util.AlertUtil;
...
public class MainActivity extends AppCompatActivity
    ...
    protected void onCreate(Bundle savedInstanceState) {
       ...
       /* if (checkPlayServices())*/
        ...
        try {
            Glide.with(MainActivity.this).load(ProjectVariables.IMAGE_PATH + PreferenceUtil.getInstance().getString(MainActivity.this, "ProfileImage", "Image_5756.jpg"))
                    .thumbnail(0.5f)
                    .crossFade()
        ...
        if (UserRole.equalsIgnoreCase("3")) {//User
            ...
            MenuItem rep = m.findItem(R.id.nav_sectionIncharge);
            rep.setVisible(false);

            MenuItem feeding = m.findItem(R.id.nav_feeding_paper);
            feeding.setVisible(false);

            MenuItem proforma = m.findItem(R.id.nav_proforma);
            proforma.setVisible(false);


        } else if (UserRole.equalsIgnoreCase("5")) {//Manger
            Menu m = navigationView.getMenu();
        ...
        } else if (UserRole.equalsIgnoreCase("6")) {//Purchase Incharge
            Menu m = navigationView.getMenu();
        ...
        if (UserRole.equalsIgnoreCase("4")) {//Admin
            Menu m = navigationView.getMenu();
...
/*    private boolean checkPlayServices() {
    ...
    private void callAsynchronousTask() {
                    ...
                    public void run() {
                        if (currentdate != null) {
                            try {
                                ...
                                try {
                                    obj.accumulate("IMEID", currentdate);
                                    Log.e("IMEID :", currentdate);
                                } catch (JSONException e) {
                                    e.printStackTrace();
                                }
                                try {
                                    AsynHttpPost143 post = new AsynHttpPost143(MainActivity.this, 0, 145, ProjectVariables.TASK_AUTORESEND, listener, obj, "", true);
                                    post.execute();
                                } catch (Exception e) {
                                    e.printStackTrace();
                                }
        ...
        timer.schedule(doAsynchronousTask, 0, 7000000); //864000000
    }
    ...
    public boolean onNavigationItemSelected(MenuItem item) {
        ...
        } else if (id == R.id.nav_sectionIncharge) {
            SectionInchargesFragment f = SectionInchargesFragment.newInstance();
            FragmentManager fm = getSupportFragmentManager();
            FragmentTransaction ft = fm.beginTransaction();
            ft.replace(R.id.container, f);
            ft.commit();

        } else if (id == R.id.nav_feeding_paper) {
            feedingPaperFragment f = feedingPaperFragment.newInstance();
            FragmentManager fm = getSupportFragmentManager();
            FragmentTransaction ft = fm.beginTransaction();
            ft.replace(R.id.container, f);
            ft.commit();

        } else if (id == R.id.nav_purchaeOrder) {
            DailyReportFragment f = DailyReportFragment.newInstance();
            FragmentManager fm = getSupportFragmentManager();
            FragmentTransaction ft = fm.beginTransaction();
            ft.replace(R.id.container, f);
            ft.commit();

        } else if (id == R.id.nav_proforma) {
            ProformaFragment f = ProformaFragment.newInstance();
            FragmentManager fm = getSupportFragmentManager();
            FragmentTransaction ft = fm.beginTransaction();
            ft.replace(R.id.container, f);
            ft.commit();
    ...
    public void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (resultCode == Activity.RESULT_OK) {
            ...
            if (requestCode == 667) {
                ...
                //UploadTask u = null;
                try {
                    ImageUploadTask uploadTask = new ImageUploadTask(MainActivity.this, getContentResolver().openInputStream(selectedimg), convertedImage);
                    uploadTask.execute();
